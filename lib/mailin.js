...
var Mailin = function () {
    ...
    this.configuration = {
        ...
        log: {
            enable: true,
            logFile: "logs/mailin.log",
            level: "info"
        },
        spam: {
            enable: true,
            provider: 'spamassasin'
        },
        dns: {
            checkDKIM: true,
            checkSPF: true,
...
Mailin.prototype.start = function (options, callback) {
    ...
    if (configuration.webhook.enable) {
        var url = configuration.webhook.url;
        request
            .head(url)
            .timeout(3000)
            .end(function (err, resp) {
                if (err || resp.statusCode !== 200) {
                    ...
                    logger.info('Webhook ' + url + ' is valid, up and running.');
                }
            });
    }
    ...
    function validateAddress(addressType, email, envelope) {
        return new Promise(function (resolve, reject) {
            ...
            try {
                ...
                if (!configuration.dns.validateDNS) {
                    validateViaLocal();
                } else {
                    validateViaDNS();
                }
    ...
    function validateDkim(connection, rawEmail) {
        if (!configuration.dns.checkDKIM) {
    ...
    function computeSpamScore(connection, rawEmail) {
        if (!configuration.spam.enable) {
            return Promise.resolve(0.0);
        }

        return mailUtilities.computeSpamScoreAsync(rawEmail, configuration.spamProvider)
            .catch(function (err) {
    ...
    function postWebhook(connection, finalizedMessage) {
        return new Promise(function (resolve) {
            if (!configuration.webhook.enable) return resolve();

            logger.info(connection.id + ' Sending request to webhook ' + configuration.webhook.url);
            ...
            var req = request.post(configuration.webhook.url);
            ...
            req.end(function (err, resp) {
                ...
                if (err || resp.statusCode !== 200) {
                    logger.error(connection.id + ' Error in posting to webhook ' + configuration.webhook.url);
                    if (resp) {
                ...
                logger.info(connection.id + ' Succesfully posted to webhook ' + configuration.webhook.url);
